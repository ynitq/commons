...
public class IPSeeker {
	...
	private final Map<String, IPLocation> ipCache = new LRULinkedHashMap<String, IPLocation>(1000);
	// 随机文件访问类
	...
	private void init() {
		URL fileUrl = this.getClass().getClassLoader().getResource(IP_FILE);
		if (fileUrl != null) {
			try {
				String fileName = "/" + fileUrl.toExternalForm().substring(6);
				...
				if (ipBegin == -1 || ipEnd == -1) {
					...
					log.error("IP地址库文件 " + IP_FILE + " 文件格式错误");
				}
	...
	private String getCountry(byte[] ip) {
		// 检查ip地址文件是否正常
		if (ipFile == null)
			return IPSeekerConstants.bad_ip_file;
		// 保存ip，转换ip字节数组为字符串形式
	...
	private IPLocation getIPLocation(byte[] ip) {
		...
		if (info == null) {
			info = new IPLocation();
			info.setCountry(IPSeekerConstants.unknown_country);
			info.setArea(IPSeekerConstants.unknown_area);
		}
		return info;
	}
	...
	private String readArea(long offset) throws IOException {
		...
		if (b == REDIRECT_MODE_1 || b == REDIRECT_MODE_2) {
			...
			if (areaOffset == 0)
				return IPSeekerConstants.unknown_area;
			else
